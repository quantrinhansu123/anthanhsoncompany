...
BEGIN
  ...
  -- 2 + 3 chỉ thực hiện khi bảng du_an đã tồn tại
  IF EXISTS (
    SELECT 1
    FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'du_an'
  ) THEN
    -- Migration dữ liệu: map project_name sang du_an_id
...
BEGIN
  ...
  -- Gỡ customer_id mồ côi trước khi gắn FK (tránh 23503)
...
BEGIN
  ...
  -- Tên gói thầu (theo phiếu; tách với hop_dong khi cần)
  IF NOT EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'thu_chi' 
