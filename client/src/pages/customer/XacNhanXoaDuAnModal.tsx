...
interface XacNhanXoaDuAnModalProps {
    isOpen: boolean;
    onClose: () => void;
...
export function XacNhanXoaDuAnModal({ isOpen, onClose, project, onSuccess }: XacNhanXoaDuAnModalProps) {
    ...
    const handleConfirm = async () => {
        ...
        setError(null);
        try {
            const success = await projectService.delete(String(project.id));
            ...
            console.error('[XacNhanXoaDuAnModal] Error deleting project:', err);
                ...
                <div className="px-6 py-6 pb-2">
                    <div className="text-center space-y-3">
                        <p className="text-slate-600">
                            Bạn có chắc chắn muốn xóa dự án <span className="font-bold text-slate-800">{project?.projectName}</span> không?
                        </p>
                        <div className="p-3 bg-amber-50 rounded-xl border border-amber-100 flex items-start gap-3">
                            <AlertTriangle size={18} className="text-amber-600 shrink-0 mt-0.5" />
                            <p className="text-[12px] text-amber-800 text-left leading-relaxed">
                                Hành động này <span className="font-bold">không thể hoàn tác</span>. Toàn bộ hợp đồng, công việc và dữ liệu liên quan sẽ bị xóa vĩnh viễn.
                            </p>
                        </div>
                    </div>
